...
# Load the dataset
breast_data <- read_excel("E:/Data Science and Business Informatics/Statistics for Data Science/Dataset/breast (2) - Copy.xlsx")

# Ensure Class is a factor
breast_data$Class <- as.factor(breast_data$Class)
...
CLASS_1 <- "benign"                # Labels for the two classes in the dataset
CLASS_2 <- "malignant"
...
em_algorithm <- function(prob_df, pt_omega1, pt_omega2, tol = 1e-6, max_iter = 100) {
  ...
  # pt_omega1: Initializes the prior probability for class w1 with the provided starting value.
  p_omega1 <- pt_omega1
  # pt_omega2: Initializes the prior probability for class w2
  p_omega2 <- pt_omega2
...
for (selection in 1:num_selections) {
  ...
  training_set <- generate_training_set(breast_data, train_size_per_class, seed_value + selection)
  test_set <- generate_test_set_from_remaining(breast_data, training_set, prior_probs[1], seed_value + selection)
  ...
  for (rep in 1:num_replications) {
    ...
    train_inputs <- as.matrix(training_set %>% select(-1, -Class))
    ...
    for (prior_prob in prior_probs) {
      ...
      set.seed(seed_value + selection * 100 + rep)
      test_set <- generate_test_set_from_remaining(breast_data, training_set, prior_prob, seed_value + selection * 100 + rep)
      test_inputs <- as.matrix(test_set %>% select(-1, -Class))
      test_actual <- test_set$Class
