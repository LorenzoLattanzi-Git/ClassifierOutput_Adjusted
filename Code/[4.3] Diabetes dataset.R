...
library(nnet)
# Load the dataset
diabetes_data <- read_csv("E:/Data Science and Business Informatics/Statistics for Data Science/Dataset/diabetes.csv")
...
CLASS_1 <- "0"                # Labels for the two classes in the dataset
CLASS_2 <- "1"
...
for (selection in 1:num_selections) {
  ...
  training_set <- generate_training_set(diabetes_data, train_size_per_class, seed_value + selection)
  test_set <- generate_test_set_from_remaining(diabetes_data, training_set, prior_probs[1], seed_value + selection)
  ...
  for (rep in 1:num_replications) {
    ...
    for (prior_prob in prior_probs) {
      ...
      set.seed(seed_value + selection * 100 + rep)
      test_set <- generate_test_set_from_remaining(diabetes_data, training_set, prior_prob, seed_value + selection * 100 + rep)
      test_inputs <- as.matrix(test_set %>% select(-Class))
      test_actual <- test_set$Class
